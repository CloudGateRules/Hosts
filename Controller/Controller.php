...
$Potatso_OtherA        = preg_replace('/([^])([ \s]+)/','  - $1$2',$OtherCURLF);
...
$Wingy_Default         = preg_replace('/(DOMAIN-SUFFIX,)/','      - s,',$DefaultCURLF);
$Wingy_AdvancedA       = preg_replace('/(DOMAIN-SUFFIX,)/','      - s,',$AdvancedCURLF);
$Wingy_Advanced        = preg_replace('/(DOMAIN,)/','      - c,',$Wingy_AdvancedA);
$Wingy_BasicA          = preg_replace('/(DOMAIN-SUFFIX,)','      - s,',$BasicCURLF);
$Wingy_Basic           = preg_replace('/(DOMAIN,)/','      - s,',$Wingy_BasicA);
$Wingy_REJECTA         = preg_replace('/(DOMAIN,)/','      - c,',$REJECTCURLF);
$Wingy_REJECT          = preg_replace('/(DOMAIN-SUFFIX,)/','      - s,',$Wingy_REJECTA);
$Wingy_DIRECTA         = preg_replace('/(DOMAIN,)/','      - c,',$DIRECTCURLF);
...
$Hosts_Format_ReplaceA = preg_replace('/(\d+\.\d+\.\d+\.\d+)([ \t]+)([a-z0-9_\-\/.%]+)/','$3 = $1',$HostsCURLF);
